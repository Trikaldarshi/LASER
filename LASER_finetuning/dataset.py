...
Code is adapted from https://pytorch.org/audio/main/_modules/torchaudio/datasets/librispeech.html#LIBRISPEECH
...
transform1 = torchaudio.transforms.PitchShift(16000, n_steps=2)
transform2 = torchaudio.transforms.PitchShift(16000, n_steps=-2)

## For HuBERT
speed_factors = [0.9, 1.0, 1.1]
# ## For WavLM
# speed_factors =[[0.80, 0.85, 0.9, 1.0, 1.1, 1.15, 1.20]]
...
class LIBRISPEECH(Dataset):
    ...
    def __getitem__(self, n: int) -> Tuple[Tensor, int, str, int, int, int]:
        ...
        wav2, _ = speed_perturb(waveform)

        # if 0 then pitch shift up, if 1 then pitch shift down
        if random.random() < 0.5:
            wav2 = transform1(wav2)
        else:
            wav2 = transform2(wav2)
        wav2 = wav2.detach()


        if random.random() < 0.5:
            return (wav2,) + (waveform,)
        else:
            return (waveform,) + (wav2,)
